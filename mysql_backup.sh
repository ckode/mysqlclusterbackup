...
# For Authentication
#
# Do not use the username & password below
# it is less secure.
#
# Add a user to your /etc/my.cnf under
# the [client] section as seen below.
#
# Insure your /etc/my.cnf has 600 permissions
# and owned by root.
##############################################
#
# [client]
...
# +----------------------------------------------------------------------------------------------------------------------------------------------------------+
# | GRANT SELECT, INSERT, CREATE, RELOAD, PROCESS, SUPER, LOCK TABLES, REPLICATION CLIENT, SHOW VIEW, CREATE TABLESPACE ON *.* TO `backup_user`@`localhost` |
# | GRANT BACKUP_ADMIN ON *.* TO `backup_user`@`localhost`                                                                                                  |
# +----------------------------------------------------------------------------------------------------------------------------------------------------------+
#
##############################################
# DO NOT USE USERNAME AND PASSWORD (see above)
...
DBHOST=localhost
# Use this as teh hostname if DBHOST is set to localhost to identify the current server in the email / logs
ALTHOSTNAME=myserver
# List of DBNAMES for Daily/Weekly Backup e.g. "DB1 DB2 DB3"
DBNAMES="all"

# Backup directory location e.g /backups
#BACKUPDIR="/opt/dbbackups"
BACKUPDIR="/mnt/dbbackups"
...
MAILADDR="to-address@example.com"
MAILFROM="from-address@example.com"
# ============================================================
...
OPT="--ignore-table=trade.invoice_data_raw --ignore-table=trade.trade_listing --ignore-table=trade.user_view --quote-names --opt --single-transaction"  # OPT string for use with mysqldump ( see man mysqldump )
...
then
	cat "$LOGFILE" | mail -s "MySQL Backup Log for $HOST - $DATE" -r $MAILFROM  $MAILADDR
else
	cat "$LOGFILE"
fi
